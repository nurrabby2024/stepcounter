"""Output handling for StepCounter."""
...
class OutputHandler:
 """Processes output requests with retries and timeouts."""
 ...
 if not payload:
 raise ValueError("empty output payload")
 return {"output": str(payload)[:200], "attempts": 0}
...
def run_output(payload, **options):
 """Module-level convenience entry point."""
 return OutputHandler().run(payload, **options)